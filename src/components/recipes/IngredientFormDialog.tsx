...
export default function IngredientFormDialog({ open, onOpenChange, ingredient }: Props) {
  ...
  // Also detect container on the main Unit (base_unit_id) field — the spec
  // requires the conversion section to appear under "Unit" for Package/Bao etc.
  const selectedBaseUnit = useMemo(
    () => units.find((u) => u.id === form.base_unit_id) ?? null,
    [units, form.base_unit_id],
  );
  const baseUnitLabel = selectedBaseUnit
    ? (selectedBaseUnit.name_vi ? `${selectedBaseUnit.name_en}/${selectedBaseUnit.name_vi}` : selectedBaseUnit.name_en)
    : '';
  const isContainerBase =
    isContainerUnitName(baseUnitLabel) ||
    isContainerUnitName(selectedBaseUnit?.name_en) ||
    isContainerUnitName((selectedBaseUnit as any)?.name_vi) ||
    isContainerUnitName((selectedBaseUnit as any)?.code);
  const showConversionSection = isContainerBase || isContainerPurchase;
  // Label of the unit the user actually picked (prefer base unit since that's
  // the visible "Unit" field; fall back to purchase unit if only that is set).
  const conversionSourceUnitLabel = isContainerBase
    ? baseUnitLabel
    : (purchaseUnitLabel || baseUnitLabel);
