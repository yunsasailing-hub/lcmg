  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Use this only when an account email must be corrected or migrated. Wrong changes can block login.
        </AlertDescription>
      </Alert>
      ...
      <div className="rounded-lg border bg-card p-4 space-y-4">
        ...
        {selected && (
          <div className="rounded-md border bg-muted/40 p-3 text-sm grid grid-cols-2 gap-2">
            ...
            <div className="col-span-2"><span className="text-muted-foreground">Branch: </span>{selected.branch_id ? (branchMap[selected.branch_id] || selected.branch_id) : 'No branch'}</div>
          </div>
        )}
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        {!logs || logs.length === 0 ? (
          <p className="text-sm text-muted-foreground">No email changes recorded yet.</p>
        ) : (
              ...
              <TableBody>
                {logs.map((l: any) => (
                  <TableRow key={l.id}>
                    ...
                    <TableCell className="text-xs max-w-[200px] truncate" title={l.reason}>{l.reason}</TableCell>
                    <TableCell>
                      <Badge
                        className={
